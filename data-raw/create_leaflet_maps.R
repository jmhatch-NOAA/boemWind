...
leaflet_weas <- leaflet::leaflet() |>
  ...
  leaflet::addPolygons(data = weas |> dplyr::filter(LEASE_STAGE == 'Active'), group = 'Active Leases', popup = ~POPUP, color = '#FF4438') |>
  leaflet::addPolygons(data = weas |> dplyr::filter(LEASE_STAGE == 'Planning'), group = 'Planning Areas', popup = ~POPUP, color = '#FF8300') |>
...
# save
htmlwidgets::saveWidget(widget = leaflet_weas, file = here::here('leaflet_maps', 'boem_weas.html'), title = 'Map • boemWind')
...
writeLines(wea_out, here::here('leaflet_maps', 'boem_areas.html'))
